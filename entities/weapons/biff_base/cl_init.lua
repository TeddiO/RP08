...
function SWEP:DrawHUD()
	...
	if self.HasCrossHair and not self.Weapon:GetNetworkedBool("Ironsights") then

		local x = ScrW() / 2.0
		local y = ScrH() / 2.0
		local scale = 10 * self.CurrentSpread
...
function SWEP:AdjustMouseSensitivity()
	local plysens = GetConVarNumber("sensitivity")
	local zoomlvl = self.Weapon:GetNetworkedFloat("ScopeZoom")
	if self.Weapon:GetNetworkedBool("Scope") then
		return math.Clamp( ( plysens / ( 2 + zoomlvl ) ), 0.1, 10 )
	end
	return -1
end
