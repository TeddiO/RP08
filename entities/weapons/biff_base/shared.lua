...
SWEP.Author			= "Teta_Bonita & PC Camp"
SWEP.Contact		= ""
...
SWEP.GassedModifier		= 3 --bit too much? idk
...
SWEP.FireModes.Grenade.FireFunction = function(self)
	...
	if SERVER then
		...
		grenade:DisallowDeleting( true )
		grenade:DisallowMoving( true )
		grenade:Spawn()
...
function SWEP:Deploy() --[[ phoenix - whole function added
	self.walkspeed = math.Clamp( 300 - ( self.Weight * 10 ), 50, 1000 )
	self.sprintspeed = math.Clamp( 400 - ( self.Weight * 10 ), 50, 1000 )]]
...
function SWEP:Holster() -- phoenix - whole function added
	self.Weapon:SetNWBool( "silenced", false )
	if (SERVER) then
...
function SWEP:BaseAttack()
	...
	if not self.OwnerIsNPC then -- NPCs don't get modifiers
		...
		modifier = self:CalculateModifiers(self.RunModifier,self.CrouchModifier,self.JumpModifier,self.IronSightModifier,self.GassedModifier)
...
function SWEP:RGShootBullet(dmg, speed, spread, spray, sprayvec, numbul, accel, mask, filter)
	...
	numbul = numbul or 1
	accel = Vector(0,0,0) -- Gravity PC Camp here -600
	...
	if SERVER then
		for i=1,numbul do
			...
			tBullet.Force	= 5*dmg -- 0.25 PC Camp
			tBullet.Damage	= dmg
...
function SWEP:RGShootBulletCheap(dmg, speed, spread, spray, sprayvec, numbul)
	...
	bullet.Dir		= (PlayerAim + 0.04*spray*sprayvec:GetNormalized()):GetNormalized() -- phoenix - PlayerAim
	bullet.Spread	= Vector(spread, spread, 0) -- phoenix - Vector(spread, spread, 0)
	bullet.Force	= 5*dmg
	bullet.Damage	= dmg
	bullet.Tracer	= 0
...
function SWEP:CalculateModifiers(run,crouch,jump,iron,gassed)
	...
	if self.Owner:GetTable().BeingGassed==true then
		modifier=modifier*gassed
	end
